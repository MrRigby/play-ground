...
public class TwitterOAuthService implements OAuthService {
	...
	public TwitterOAuthService(String consumerKey, String consumerSercet) {
		this.consumerKey = consumerKey;
		this.consumerSecret = consumerSercet;
		this.key = new ConsumerKey(this.consumerKey, this.consumerSecret);
	...
	public Promise<JsonNode> registeredUserProfile(RequestToken token,
		...
		Promise<Response> result = request.sign(new OAuthCalculator(this.key, accessToken)).get();
		Promise<JsonNode> promiseOfJson = result.map(Functions.responseToJson);
		Promise<String> promiseOfScreenName = promiseOfJson.map(Functions.findTextElement("screen_name"));
		return promiseOfScreenName.flatMap(userProfile);
	}
